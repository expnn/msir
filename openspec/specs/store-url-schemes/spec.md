# Store URL Schemes Specification

## Purpose

Define how the store builder maps URL schemes to storage protocol backends and resolves bucket, endpoint, and credentials. This covers the breaking change that `oss://` now means the Aliyun OSS native protocol with the bucket in the URL authority, while `s3://` and `http(s)://` speak the S3-compatible protocol.
