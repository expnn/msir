# Store Failure Diagnostics Specification

## Purpose

When store construction or its preflight check fails, perform one lightweight probe against the endpoint and augment the error with a protocol-mismatch hint (e.g. Aliyun OSS reached via an `s3://` URL). The probe is diagnostic only — it never switches the protocol backend.
