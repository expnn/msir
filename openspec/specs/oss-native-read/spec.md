# OSS Native Read Specification

## Purpose

Enable reading zarr datasets from real Aliyun OSS via the native OSS protocol (OpenDAL `services-oss` backend), with virtual-host addressing, credential preflight checks, and the existing list-quirk normalization retained.
