...
def write_variant(out_dir: Path, name: str, M: int, X: int, config: str,
                  shard_samples: int, shard_sp: int,
                  flux_all: np.ndarray, ivar_all, mask_all) -> None:
    ...
    g.create_array("flux", shape=(0, bands, H, W), chunks=(M, bands, X, X),
                   shards=flux_shards, dtype="float32", chunk_key_encoding=cke,
                   compressors=compressors)
    if ivar_all is not None:
        g.create_array("ivar", shape=(0, bands, H, W), chunks=(M, bands, X, X),
                       shards=flux_shards, dtype="float32", chunk_key_encoding=cke,
                       compressors=compressors)
    if mask_all is not None:
        if mask_all.ndim == 3:  # (n, H, W)
            ...
            g.create_array("mask", shape=(0, bands, H, W),
                           chunks=(M, bands, X, X), shards=flux_shards,
                           dtype="bool", fill_value=True, chunk_key_encoding=cke,
                           compressors=compressors)
    zf = g["flux"]
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Zarr benchmark-data generator with auto-computed sharding spatial.")
    ...
    ap.add_argument("--n-samples", type=int, default=4096,
                    help="samples to copy from source.")
    ap.add_argument("--chunk-samples", type=int, nargs="+", default=[8, 16, 32],
                    help="chunk sample-dim sizes M (cartesian axis). "
                         "Shard sample-dim N must be divisible by each M.")
    ap.add_argument("--spatial", type=int, nargs="+", default=[16, 32],
                    help="spatial chunk sizes X (cartesian axis). "
                         "Shard spatial is auto-computed as max(32, X).")
    ...
    ap.add_argument("--shard-samples", type=int, default=1024,
                    help="shard sample-dim size N (only configurable shard dimension).")
    args = ap.parse_args()

    out_dir = Path(args.output_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    print(f"== gen variants: M={args.chunk_samples} X={args.spatial} "
          f"configs={args.configs} N={args.shard_samples} ==")
    ...
    for X in args.spatial:
        shard_sp = max(32, X)
        if H % X != 0 or W % X != 0:
            print(f"ERROR: chunk spatial X={X} must divide image HxW={H}x{W}",
                  file=sys.stderr)
            return 2
        if H % shard_sp != 0 or W % shard_sp != 0:
            print(f"ERROR: shard spatial max(32,{X})={shard_sp} must divide "
                  f"image HxW={H}x{W}", file=sys.stderr)
            return 2
        if shard_sp % X != 0:
            print(f"ERROR: shard spatial {shard_sp} must be a multiple of chunk "
                  f"spatial {X}", file=sys.stderr)
            return 2
    ...
    if any(c != "noshard" for c in args.configs):
        ...
        if bad:
            print(f"ERROR: M {bad} must divide --shard-samples {args.shard_samples} "
    ...
    for M in args.chunk_samples:
        for X in args.spatial:
            ...
            for cfg in args.configs:
                ...
                t0 = time.perf_counter()
                write_variant(out_dir, name, M, X, cfg, args.shard_samples,
                              shard_sp, flux, ivar, mask)
